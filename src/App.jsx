...
const translations = {
  zh: {
    ...
    sop_s1_basic: "基礎規格： 提供產品名稱/SKU、數量與材質結構",
    sop_s1_size: "精確尺寸： 提供確切的公制尺寸 (cm)",
    sop_s1_technique: "工法定義： 確認特殊印刷效果的具體類型與區域",
    sop_s1_format: "格式合規： 檔案為 AI 或 EPS 格式",
    sop_s1_font: "字體與連結： 文字皆已轉換為外框 (Outlines)",
    sop_s1_resource: "資源齊全： 外部連結資源皆已正確嵌入或附上",
    sop_s1_spell: "除錯掃描： 已完成基本拼寫檢查",
    sop_s1_dieline: "刀模防呆： 新刀模已標明英吋與公分雙軌尺寸",
    sop_s1_structure: "結構防呆： 客戶已提供新結構的參考圖片或例子",
    sop_stage2: "Stage 2: 校對與審核準備",
    sop_s2_desc: "出圖給客戶前，以及整理雙方檔案時的確認。",
    sop_s2_single: "單一事實： 確認以客戶提供的「最終檔案」為唯一生產依據",
    sop_s2_cloud: "雲端同步： 最終生產檔案已上傳至指定平台NAS",
    sop_s2_permission: "權限開通： 確認雙方對共享連結皆具備存取或編輯權限",
    sop_s2_proof: "防禦性打樣： 已將校對稿轉為 PDF 格式回傳給客戶",
    sop_s2_notice: "微調告知： 已明確列出為符合印刷要求所做的任何微小調整",
    sop_stage3: "Stage 3: 最終核准與物流",
    sop_s3_desc: "送廠印製前的最後一道鎖。",
    sop_s3_approved: "絕對核准： 已收到客戶明確回覆 Approved",
    sop_s3_logistics_inv: "物流盤點： 確認特殊分批運送或免簽收等要求",
    sop_s3_logistics_conf: "物流雙重確認： 特殊物流需求已再次向客戶確認並註記",
  ...
  en: {
    ...
    sop_s2_permission: "Permissions: Shared links accessible by both parties.",
    ...
    sop_stage3: "Stage 3: Final Approval & Logistics",
    sop_s3_desc: "Final lock before printing.",
    sop_s3_approved: "Absolute Approval: 'Approved for Production' received via email.",
    sop_s3_logistics_inv: "Logistics Inventory: Split-shipping or no-signature reqs confirmed.",
    sop_s3_logistics_conf: "Logistics Double Check: Special logistics noted in final email.",
    ...
    files: [
      {
        ...
        stage: 2,
        checklists: {
          stage1: { basic: true, size: true, technique: true, format: true, font: true, resource: true, spell: false, dieline: false, structure: false },
        ...
        versions: [
          {
            ...
            date: '2023-10-01',
            status: 'approved',
            uploader: 'design.lin@designsync.com (設計師)',
            imageUrl: 'https://images.unsplash.com/photo-1585644171281-5421da63309a?auto=format&fit=crop&w=1200&q=80',
            markers: [
              { id: 1, type: 'pin', x: 45, y: 30, author: '王經理 (客戶)', text: '結構沒問題，請推進細節設計。', resolved: true, timestamp: '10:00 AM' }
            ]
          },
          {
            versionNumber: 2,
            ...
            markers: [
              { id: 2, type: 'box', x: 55, y: 45, width: 10, height: 10, author: '陳總監 (老闆)', text: '色調與品牌規範一致，條碼區請再確認。', resolved: false, timestamp: '09:15 AM' }
            ]
          }
        ]
      }
                ...
                {activeTab === 'sop' && (
                  <div className="space-y-6">
                    ...
                    <section className={`bg-white border rounded-xl p-4 ${activeFile?.stage === 2 ? 'border-blue-400 ring-2 ring-blue-50' : ''}`}>
                      ...
                      <CheckboxItem checked={activeFile?.checklists?.stage2?.permission} onChange={() => toggleChecklist('stage2', 'permission')} label={t('sop_s2_permission')} isDisabled={activeFile?.stage !== 2} isCurrentStage={activeFile?.stage === 2} />
                    ...
                    <section className={`bg-white border rounded-xl p-4 ${activeFile?.stage === 3 ? 'border-blue-400 ring-2 ring-blue-50' : ''}`}>
                      ...
                      <CheckboxItem checked={activeFile?.checklists?.stage3?.logisticsInv} onChange={() => toggleChecklist('stage3', 'logisticsInv')} label={t('sop_s3_logistics_inv')} isDisabled={activeFile?.stage !== 3} isCurrentStage={activeFile?.stage === 3} />
                      <CheckboxItem checked={activeFile?.checklists?.stage3?.logisticsConf} onChange={() => toggleChecklist('stage3', 'logisticsConf')} label={t('sop_s3_logistics_conf')} isDisabled={activeFile?.stage !== 3} isCurrentStage={activeFile?.stage === 3} />
                    </section>
